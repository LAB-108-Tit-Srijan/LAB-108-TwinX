...
class _AiChatScreenState extends State<AiChatScreen> with TickerProviderStateMixin {
  ...
  void _generateAIResponse(String userQuery) {
    ...
    if (userQuery.toLowerCase().contains('photosynthesis') || 
        userQuery.toLowerCase().contains('plant')) {
      response = '''Of course! 🌱

**Photosynthesis** is how plants make their food using:

• ☀️ **Sunlight** - Energy source
• 💧 **Water** - From roots
• 🌫️ **Carbon dioxide** - From air

The plant converts these into **glucose** (sugar) for energy and releases **oxygen** as a byproduct.

Think of it like a plant's kitchen where sunlight is the chef! 👨‍🍳''';
    } else if (userQuery.toLowerCase().contains('math') || 
               userQuery.toLowerCase().contains('equation') ||
               userQuery.toLowerCase().contains('solve')) {
      response = '''I'd love to help with math! 📐
...
Share the specific problem and I'll walk you through it step by step! 💪''';
    } else if (userQuery.toLowerCase().contains('history') || 
               userQuery.toLowerCase().contains('war') ||
               userQuery.toLowerCase().contains('civilization')) {
      response = '''Great topic! 📜

**History helps us understand:**

• 🏛️ **Past civilizations** - How they lived and learned
• ⚔️ **Major events** - Wars, discoveries, revolutions
• 👥 **Important figures** - Leaders, inventors, thinkers

What specific era or event would you like to explore? I can create a timeline or summary! 🗺️''';
    } else if (userQuery.toLowerCase().contains('physics') || 
               userQuery.toLowerCase().contains('force') ||
               userQuery.toLowerCase().contains('motion')) {
      response = '''Physics is fascinating! ⚡

**Newton's Laws of Motion:**

• 🎯 **1st Law** - Objects stay at rest or in motion unless acted upon
• 💨 **2nd Law** - F = ma (Force = Mass × Acceleration)
• 🔄 **3rd Law** - Every action has an equal opposite reaction

Want me to explain any of these with real-world examples? 🚀''';
    } else if (userQuery.toLowerCase().contains('chemistry') || 
               userQuery.toLowerCase().contains('element') ||
               userQuery.toLowerCase().contains('atom')) {
      response = '''Chemistry is amazing! 🧪

**Basic concepts:**

• ⚛️ **Atoms** - Building blocks of matter
• 🔗 **Molecules** - Atoms bonded together
• 📊 **Periodic Table** - Organized elements

**Fun fact:** Your body is made of about 7 octillion atoms! 🤯
...
• 📖 **Explaining concepts** - Making complex topics simple
• 🧩 **Problem solving** - Step-by-step solutions
• 📝 **Study notes** - Summaries and key points
• ❓ **Quiz practice** - Test your knowledge

What would you like to focus on? I'm here to help! ✨''';
    }
