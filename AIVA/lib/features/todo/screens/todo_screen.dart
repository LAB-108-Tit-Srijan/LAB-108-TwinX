...
class TodoItem {
  ...
  TodoItem copyWith({bool? isCompleted, String? title}) {
    return TodoItem(
      id: id,
  ...
  factory TodoItem.fromApi(Map<String, dynamic> json) {
    return TodoItem(
      id: json['id']?.toString() ?? json['_id']?.toString() ?? '',
      title: json['title']?.toString() ?? '',
      description: json['description']?.toString(),
      category: json['category']?.toString() ?? 'Study',
...
class _TodoScreenState extends State<TodoScreen> {
  ...
  Future<void> _loadTodos() async {
    ...
    try {
      final raw = await TodoService.getTodos();
      if (mounted) {
        setState(() {
          _todos = raw
              .map((e) => TodoItem.fromApi(e as Map<String, dynamic>))
              .toList();
          _isLoading = false;
        });
      }
  ...
  Future<void> _toggleCompleted(TodoItem todo) async {
    ...
    if (idx == -1) return;
    setState(() {
      _todos[idx] = todo.copyWith(isCompleted: !todo.isCompleted);
    });
    try {
      await TodoService.updateTodo(todo.id, completed: !todo.isCompleted);
    } catch (e) {
      // Revert on error
      if (mounted) {
        setState(() {
  ...
  Future<void> _deleteTodo(TodoItem todo) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _todos.insert(idx, todo));
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            Expanded(
              child: _isLoading
                  ...
                  : RefreshIndicator(
                      ...
                      color: AppColors.primaryDark,
                      child: _buildTodoList(dvpw, dvph),
                    ),
            ),
          ],
        ),
      ...
      itemBuilder: (context, index) {
        return Dismissible(
          ...
          background: Container(
            ...
            padding: EdgeInsets.only(right: dvpw * 0.05),
            decoration: BoxDecoration(
              color: AppColors.red,
              borderRadius: BorderRadius.circular(dvpw * 0.04),
            ),
            child: Icon(Icons.delete_rounded, color: AppColors.white, size: dvpw * 0.07),
          ),
          onDismissed: (_) => _deleteTodo(todos[index]),
          child: _buildTodoCard(todos[index], dvpw, dvph),
        );
      },
    );
  }
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              if (titleController.text.isNotEmpty) {
                                ...
                                try {
                                  final result = await TodoService.createTodo(titleController.text);
                                  if (result['success'] == true || result['todo'] != null) {
                                    await _loadTodos();
                                  } else {
