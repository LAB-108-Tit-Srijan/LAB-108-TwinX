...
class _RoadmapScreenState extends State<RoadmapScreen> {
  ...
  void initState() {
    ...
    if (widget.enrollmentId != null) {
      if (widget.generatedPlan != null) {
        _roadmap = Roadmap.fromJson({'id': '', 'plan': widget.generatedPlan});
      } else {
        _loadRoadmap();
      }
    }
  ...
  Future<void> _loadRoadmap() async {
    ...
    try {
      final data = await ApiService.get('/api/roadmap/${widget.enrollmentId}');
      if (data['success'] == true) {
        setState(() => _roadmap = Roadmap.fromJson(data['roadmap'] as Map<String, dynamic>));
      }
    } catch (_) {
    } finally {
  ...
  Widget _buildRoadmapContent(double dvpw, double dvph) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // AI Roadmap from API (if available)
          if (widget.enrollmentId != null) ...[
            if (_isLoadingRoadmap)
              const LoadingWidget()
            else if (_roadmap != null) ...[
              _buildAIRoadmapCard(_roadmap!, dvpw, dvph),
  ...
  Widget _buildAIRoadmapCard(Roadmap roadmap, double dvpw, double dvph) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Container(
              padding: EdgeInsets.all(dvpw * 0.025),
              decoration: BoxDecoration(color: AppColors.primaryLime, borderRadius: BorderRadius.circular(dvpw * 0.025)),
              child: Icon(Icons.auto_awesome_rounded, size: dvpw * 0.05, color: AppColors.primaryDark),
            ),
            SizedBox(width: dvpw * 0.03),
            Text('AIVA Study Plan', style: GoogleFonts.lato(fontSize: dvpw * 0.04, fontWeight: FontWeight.w700, color: Colors.white)),
          ]),
          SizedBox(height: dvph * 0.015),
          Text(roadmap.overview, style: GoogleFonts.lato(fontSize: dvpw * 0.034, color: Colors.white70, height: 1.5)),
          SizedBox(height: dvph * 0.015),
          Wrap(
            spacing: dvpw * 0.02,
            children: [
              _infoChip('${roadmap.dailyGoalHours}h/day', dvpw),
              _infoChip('${roadmap.estimatedDays} days', dvpw),
            ],
          ),
          if (roadmap.tips.isNotEmpty) ...[
            SizedBox(height: dvph * 0.015),
            Text('Tips', style: GoogleFonts.lato(fontSize: dvpw * 0.035, fontWeight: FontWeight.w600, color: AppColors.primaryLime)),
            SizedBox(height: dvph * 0.008),
            Wrap(
              spacing: dvpw * 0.02,
              runSpacing: dvpw * 0.015,
              children: roadmap.tips.map((tip) => Container(
                padding: EdgeInsets.symmetric(horizontal: dvpw * 0.03, vertical: dvpw * 0.015),
                decoration: BoxDecoration(
                  color: Colors.white.withOpacity(0.08),
                  borderRadius: BorderRadius.circular(dvpw * 0.02),
                ),
                child: Text(tip, style: GoogleFonts.lato(fontSize: dvpw * 0.03, color: Colors.white70)),
              )).toList(),
            ),
          ],
        ],
  ...
  Widget _infoChip(String label, double dvpw) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: dvpw * 0.03, vertical: dvpw * 0.015),
      decoration: BoxDecoration(
      ...
      child: Text(label, style: GoogleFonts.lato(fontSize: dvpw * 0.032, color: AppColors.primaryLime, fontWeight: FontWeight.w600)),
    );
  }
